...
(function ($, undefined) {
	...
	// TODO javadoc
	function tags(data) {
		// TODO even for a function we should probably return a wrapper that does the same object/string check as the function for arrays. otherwise only functions that return objects are supported.
		if ($.isFunction(data)) {
			return data;
		}
	...
	AbstractSelect2 = clazz(Object, {
		...
		init:       function (opts) {
			...
			if (opts.noSearch) {
				this.dropdown.find('.select2-search').hide();
			}
			...
			opts.query({
				...
				callback:this.bind(function (data) {
					...
					if (this.opts.createSearchChoice && search.val() !== "") {
						...
						if (def.message) {
							render("<li class='select2-no-results'>" + def.message + "</li>");
							return;
						} else if (def !== undefined && def !== null && self.id(def) !== undefined && self.id(def) !== null) {
